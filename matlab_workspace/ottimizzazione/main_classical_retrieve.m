%Funzione per ottenere i parametri dinamici del cobot FRANKA 
%richiede nel caso del metodo RE 2_Step di settare use_complete_regressor = false;
%richiede nel caso del metodo OSI di settare use_complete_regressor = true;
%e il regressore completo insieme a tau_stack
% funzione creata con matlab 2021_a il 05_08_2022 da Gatti Gabriele e Riccardo Bisogni.
%usa come base di partenza il codice di:
%  Claudio Gaz, Marco Cognetti
% from
...
% global variables
global SA_step Y_stack_LI tau_stack P_li_full_subs num_of_samples use_complete_regressor PI_RE
...
%% load data and inizializzation
if(use_complete_regressor)
    load("Y_e_tau_class.mat");
...
% ---------------------------
if use_complete_regressor
   [LB,UB] = read_bounds('BBB.csv'); 
else
    [LB,UB] = read_bounds('bounds_re.csv');
end
LB(7)=0.7; %l'end effector pesa almeno 0.7KG;
LB(28)=0.5;
%----------------------------------
% initializations
%----------------------------------
num_of_runs = 15; % partenze indipendenti dell'algoritmo di ottimizzazione
num_of_SA_steps = 7; % miglioramenti successivi al primo step indipendente
...
for i=1:num_of_runs
    for SA_step=1:num_of_SA_steps
        ...
        options = saoptimset('HybridFcn',{@fmincon},'MaxFunEvals',1e+04); % use Nelder-Mead optimization as hybrid function

        if use_complete_regressor
            [X,FVAL,EXITFLAG,OUTPUT] = simulannealbnd(@(x) error_fcn_gM_LMI_regressor(x),X0,LB,UB,options);
        else
            [X,FVAL,EXITFLAG,OUTPUT] = simulannealbnd(@(x) errore_reverse_eng(x),X0,LB,UB,options);
        end
...
if use_complete_regressor %se è completo considero anche la friction_ altrimenti uso un altro modello di friction
...
P_li_expanded_eval =compute_PI_classic_completo_simplified(I2xx,I2xy,I3xx,I2xz,I3xy,I4xx,I3xz,I4xy,I5xx,I4xz,I5xy,I6xx,I5xz,I6xy,I7xx,I6xz,I7xy,I7xz,I2yy,I2yz,I3yy,I3yz,I4yy,I4yz,I5yy,I5yz,I6yy,I6yz,I7yy,I7yz,I1zz,I2zz,I3zz,I4zz,I5zz,I6zz,I7zz,c1x,c2x,c3x,c4x,c5x,c6x,c7x,c1y,c2y,c3y,c4y,c5y,c6y,c7y,c2z,c3z,c4z,c5z,c6z,c7z,fo1,fo2,fo3,fo4,fo5,fo6,fo7,fc1,fc2,fc3,fc4,fc5,fc6,fc7,fv1,fv2,fv3,fv4,fv5,fv6,fv7,m1,m2,m3,m4,m5,m6,m7);
% joint torques estimation
if use_complete_regressor
save("res_osi.mat", "optimal_solution", "P_li_expanded_eval");
...
save("res_p^", "optimal_solution", "P_li_expanded_eval");
end