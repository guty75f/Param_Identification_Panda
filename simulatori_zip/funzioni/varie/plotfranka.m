...
% plotfranka is like SLPLOTBOT	S-function for robot animation but with q0
% with initial position of the franka robot.
%
...
function [sys,x0,str,ts] = plotfranka(t,x,u,flag, robot, fps, holdplot)
	switch flag
	...
	case 0
        ...
        if ~isempty(robot)
            clf
            robot.plot([0 -0.7854 0 -2.3562 0 1.5708 0.7854], 'delay', 0, 'noraise')
        end
        if holdplot
            hold on
        end
